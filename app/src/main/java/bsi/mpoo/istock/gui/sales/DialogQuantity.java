...
public class DialogQuantity {
    ...
    public void invokeQuantity(final long quantityMax, final Item item, final ProductCartPredictListAdapter adapter, final ProductCartListAdapter productCartListAdapter, final TextView textView) {
        long quantity = 0;
        AlertDialog.Builder builder = new AlertDialog.Builder(context);
        LayoutInflater inflater = LayoutInflater.from(context.getApplicationContext());
        View view = inflater.inflate(R.layout.quantity_order, null, false);
        ...
        builder.setPositiveButton(context.getString(R.string.okay), new DialogInterface.OnClickListener() {
            ...
            public void onClick(DialogInterface dialog, int which) {
                ...
                if (currentValue > 0){
                    item.setQuantity(currentValue);
                    Cart.getInstance().addItem(item);
                    Cart.getInstance().removePredictItem(item);
                    adapter.notifyDataSetChanged();
                    productCartListAdapter.notifyDataSetChanged();
                    textView.setText(NumberFormat.getCurrencyInstance().format(Cart.getInstance().getTotal()));
